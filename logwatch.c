...
static int init_work(struct logwatch_data* logwatch) {
    ...
    serial_char_array = (char**) malloc(
            sizeof(char*) * (logwatch->log_num + 100));
    memset(serial_char_array, 0, sizeof(char*) * (logwatch->log_num + 100));

    serial_int_array = (unsigned long*) malloc(
            sizeof(unsigned long) * (logwatch->log_num + 100));
    memset(serial_int_array, 0,
            sizeof(unsigned long) * (logwatch->log_num + 100));
    ...
    for (i = 0; i < log_count; i++) {
        ...
        serial_char_array[i] = (char*) malloc(
                sizeof(char) * (start - log_array[i] + 1));

        memset(serial_char_array[i], 0,
                sizeof(char) * (start - log_array[i] + 1));
    ...
    asprintf(&buf, "%lu(%04d-%02d-%02d %02d:%02d:%02d)",
...
/*static void parse_kmsg_to_nand(char* buf, int size, const int fd,
        const int prior) {
...
static void* start_watch_kmsg(void* param) {
    ...
    set kernel
    log buffer
    size klog_buf_len = klogctl(KLOG_WRITE, 0, 0);
    if (klog_buf_len < 1) {
    ...
    for (;;) {
        read some
        log to
        bufffer count = klogctl(KLOG_READ, buf, klog_buf_len);
        if (count < 0) {
...
static void* start_watch_kmsg(void* param) {
    ...
    if (print_fd < 0) {
        ...
        if (retval < 0)
            LOGE("Failed to write /proc/sys/kernel/printk: %s.",
                    strerror(errno));
...
static int do_work(struct logwatch_data* logwatch) {
    ...
    if (logwatch->is_enable_kmsg) {
        logwatch->kmsg_fd = open(KERNEL_LOG_NAME,
                O_RDWR | O_SYNC | O_CREAT | O_APPEND,
    ...
    if (logwatch->is_enable_logcat) {
        logwatch->logcat_fd = open(LOGCAT_LOG_NAME,
                O_RDWR | O_SYNC | O_CREAT | O_APPEND,
                S_IRUSR | S_IWUSR | S_IRGRP | S_IROTH);
